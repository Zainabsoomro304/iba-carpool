...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log('[API] Users endpoint called, action:', action);
    ...
    switch (action) {
      ...
      case 'create': {
        ...
        if (!userData.email || !userData.erp_id) {
          return res.status(400).json({ error: 'Email and ERP ID are required' });
        }
      ...
      default:
        return res.status(400).json({ error: 'Invalid action. Use: findByEmail, findByERP, create, or updatePassword' });
    }
  } catch (error: any) {
